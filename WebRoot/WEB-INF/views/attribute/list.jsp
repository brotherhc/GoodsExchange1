					...
					<ul class="nav nav-list">
						...
						<li class="">
							<a href="${ctx}/Goods/select" class="dropdown-toggle">
								<i class="icon-edit"></i>
								<span class="menu-users"> 物品管理 </span>
							</a>
						</li>
					...
					<div class="page-content">
						...
						<div class="row">
									<div class="col-xs-12">
											...
											<div class="form-group clearfix">
												...
												<form:form modelAttribute="task" id="searchForm" action="" method="post">
													...
													<div class="col-sm-2 add_btn">
														...
														<a href="${ctx}/attribute/insert/add" class="btn btn-success">
										...
										<div class="table-responsive tagnameClass">
											...
											<c:forEach var="list" items="${list.list}">
												<tbody>
													<tr>
														<td>${list.id}</td>
														<td class="tag">${list.categoryId}</td>
														<td class="click">${list.name}</td>
														<td>${list.attributeType}</td>
														<td>${list.attributeNumber}</td>
														<td>${list.status}</td>
														<%--<td>${list.updated_at}</td>--%>
														<td>
														<div class="visible-md visible-lg hidden-sm hidden-xs btn-group">
															<a href="${ctx}/attribute/add/?id=${list.id}"> <button tag="edit"  style="cursor:pointer;" classTag="修改"  class="btn btn-xs btn-info" title="修改">
																<i class="icon-edit bigger-120"></i>
															</button></a>
															<a href="${ctx}/attribute/delete/${list.id}"><button class="btn btn-xs btn-danger" classTag="删除" title="删除" >
										...
										<%--<script type="text/javascript">--%>
											<%--$(function(){--%>
												<%--var oEdit=$('.handle > a:contains("修改")');--%>
												<%--oEdit.each(function(index,elements){--%>
													<%--oEdit[index].t=true;--%>
													<%--oEdit.eq(index).on('click',function(){--%>
														<%--var oTag=$(this).parents('tr').find('.tag')--%>
														<%--var oClick=$(this).parents('tr').find('.click')--%>
														<%--if(oEdit[index].t){--%>
															<%--oTag.attr("contenteditable","true");--%>
															<%--oTag.addClass('edit');--%>
															<%--oClick.attr("contenteditable","true");--%>
															<%--oClick.addClass('edit');--%>
															<%--oEdit[index].t=false;--%>
														<%--}else{--%>
															<%--oTag.attr("contenteditable","false");--%>
															<%--oTag.removeClass('edit');--%>
															<%--oClick.attr("contenteditable","false");--%>
															<%--oClick.removeClass('edit');--%>
															<%--oEdit[index].t=true;--%>
														<%--}--%>
													<%--});--%>
												<%--});--%>
